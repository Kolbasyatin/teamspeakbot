...
//  query_type   'a2s' | 'rest' | 'bohemia'  должен совпадать с полем type внутри query_config
...
//
//fields есть только у rest и обязателен: у каждого эндпоинта свои имена полей, и угадывать их
//нельзя. Ключ — доменное имя поля, значение — путь в ответе, точка разделяет уровни вложенности.
//Опечатка в ключе роняет чтение конфига с перечислением неизвестных имён: молча она не проявилась
//бы ничем — источник настроен, эндпоинт отвечает, поле просто никогда не читается.
//У a2s и bohemia карты нет: форма ответа задана протоколом, настраивать нечего.
//  enabled      tinyint(1)                отключает источник, не удаляя строку
//
//Порт в query_config у a2s — это порт A2S-запроса, он не равен игровому порту из game_address
//(например, игровой 2001 против A2S 17771). hostAddress у bohemia — наоборот, игровой адрес,
//тот же, что в game_address: по нему сервер ищется в каталоге Bohemia.
//
//Рабочая раскладка для Arma Reforger — два источника:
//  a2s      primary,   priority 0   статус и players прямо с игрового сервера
//  bohemia  secondary, priority 1   очередь, сценарий, код прямого подключения; players проигрывают A2S
//Чем меньше priority, тем важнее: при совпадении поля побеждает меньший. Bohemia не делать primary:
//доступность каталога и токена не связана с жизнью сервера. Второстепенные опрашиваются не чаще
//MONITOR_SECONDARY_POLL_INTERVAL_MS — между опросами в слияние идёт их прошлый ответ.
