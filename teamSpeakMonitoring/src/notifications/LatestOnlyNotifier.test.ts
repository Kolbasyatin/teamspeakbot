...
test("состояние, пришедшее во время упавшей доставки, теряется — и это видно в логе", async () => {
    //Известная дырка (AGENTS.md, п. 24). Доставка упала, цикла больше нет, а накопленное
    //состояние забрать некому: ServerMonitor эмитит viewChanged только при ИЗМЕНЕНИИ, поэтому
    //следующего события может не быть вовсе. Тест фиксирует поведение как есть и проверяет,
    //что потеря хотя бы не молчаливая. Лечится периодическим подтверждением состояния.
    const inner = createControllable();
