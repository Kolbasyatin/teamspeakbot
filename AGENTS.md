...
24. **Потеря доставки не восстанавливается — ни в TeamSpeak, ни в Telegram.** Один корень:
    обновление уходит только в момент **изменения** состояния, повторять упавшую доставку некому.
    В Telegram симптом даже жёстче: `serverOnline` эмитится только на переходе статуса, поэтому
    упавшая отправка «сервер онлайн» теряется до следующего падения и подъёма того же сервера.
    Лечится периодической проверкой расхождения между «что доставлено» и «что сейчас» —
    разобрано, ждёт решений по двум вопросам, см. раздел «Следующий шаг» в [`PLAN.md`](PLAN.md).
    Ниже — детали TeamSpeak-симптома.

    **Потеря обновления при отказе доставки TeamSpeak.** `LatestOnlyNotifier` при падении доставки
    выходит из цикла, и если за это время пришло новое состояние, забрать его некому:
    `ServerMonitor` эмитит `viewChanged` только при **изменении** состояния, поэтому следующего
    события может не быть вовсе. Воспроизводимый сценарий: все серверы ушли в offline → событие →
    TeamSpeak в этот момент недоступен → доставка упала → новое состояние осталось в `pending` →
    состояние больше не меняется → в описании канала навсегда висит «online 12/64».
    Сейчас потеря пишется в лог (`Pending notification dropped after delivery failure`) и зафиксирована
    тестом, но не лечится. Правильное лечение — **периодическое подтверждение состояния**: раз
    в N секунд отправлять текущий вид независимо от изменений. Это закрывает не только эту дырку,
    но и ручную правку описания админом, и возврат TeamSpeak после долгой недоступности; а также
    снимает остаток п. 12 — эмитить вид после каждого отдельного опроса станет незачем.
