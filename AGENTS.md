  ...
  src/
    ...
    monitoring/            ← домен: что значит «следить за сервером»
      ServerMonitor.ts       владеет probes, шедулит опрос, эмитит stateUpdated (после каждого
                             ...
                             checkOnce(config) — разовый опрос мимо расписания и probe: тот же
                             pollServerSources, результат возвращается вызывающему, событий нет
    ...
    telegram/              ← адаптер Telegram
      ...
      SubscriptionCommands.ts  /start, /serverlist, /my, /status и обработка нажатий. Кнопки
                             ...
                             и StatusSource — узкие интерфейсы к зависимостям. У списка два
                             действия (листать, открыть карточку); подписка, отписка, проверка
                             и настройки — действия карточки. Проверка отвечает НОВЫМ сообщением,
                             а не правит карточку: карточка — табло, ответ на вопрос — переписка.
                             Мимо нотифаеров, как /status
      SubscriptionEvent.ts   типы уведомлений подписки: availability, roundFinish. Здесь же подписи
                             ...
                             не появляется
      ServerCardMessage.ts   карточка сервера — единственное место действий над сервером:
                             подписаться / отписаться, «🔍 Проверить», галочки по типам уведомлений.
                             Открывается из обоих списков и носит в каждой кнопке ListOrigin
                             (список, страница, поиск), чтобы «◀ К списку» вернул туда, откуда
                             пришли. Своё пространство кодов callback_data (префикс k, семь частей)
      ServerListMessage.ts   текст и клавиатура списка чистой функцией + кодирование/разбор
                             ...
                             На сервер ровно одна кнопка, и она открывает карточку — в обоих
                             списках. Здесь же ListOrigin и encodeOrigin/decodeOrigin — формат
                             «откуда пришли», общий с карточкой
      ServerStatusMessage.ts сводка «мои серверы» для /status. Момент времени — параметром,
                             поэтому текст детерминирован. Здесь же renderServerCheck — ответ
                             на разовую проверку: игроки и очередь теми же функциями, но без
                             статуса и «онлайн уже N»: probe нет, антидребезга нет, поэтому
                             молчание сервера подписано «не ответил», а не «офлайн»
    persistence/           ← адаптер БД
      ServerRepository.ts    findMonitored(): читает monitored_servers + server_query_sources
                             ...
                             findStoredById — один включённый сервер с источниками, без условия
                             на подписку: для разовой проверки по кнопке
...
- **Умолчания исходящего — в одном месте.** Разметка (`HTML`), троттлинг под лимит Bot API
  и выключенное превью ссылок ставятся централизованно: разметка и троттлинг в `TelegramSender`,
  превью — трансформером `bot.api.config.use` в `TelegramBot`. Параметр в каждом вызове `reply`
  забывается в новом месте молча, и дефект проявляется только когда в тексте окажется ссылка
  (а названия серверов почти всегда содержат приглашение в discord).
